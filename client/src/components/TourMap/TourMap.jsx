...
const TourMap = ({startPlace, places, togglePreview, setNewPreviewPlace, previewPlace }) => {
  ...
  useEffect(() => {
    ...
    setPolylineProps(places.reduce((prev, curr) => {
      prev.push(curr.position);
      return prev;
    }, [startPlace])) // startPlace is the center of the Map, here could be a empty array
  }, [places])
  ...
  >
    ...
    {places.map((place) => (
      <Marker key={place.title} position={place.position} eventHandlers={{ click: () => showPreview(place)}} icon={locationIcon}> <Tooltip>{place.title}</Tooltip> </Marker> 
    ))}
